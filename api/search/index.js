...
module.exports = async function (context, req) {
  ...
  if (!query) {
    context.res = { status: 400, body: 'Missing query' };
    return;
  }
  ...
  if (!target.length) {
    context.res = { status: 400, body: 'No jurisdictions selected' };
    return;
  }
  ...
  if (email && consent && resend) {
    try {
      ...
      context.log('Email send failed', e.message);
    }
  }
